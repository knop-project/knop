...
not #path_here->endsWith('/')
    ? #path_here->append('/')
local(libs_path)    = #path_here + `../knop9/knoplibs/`
...
//   $argv->first->endsWith(loop_value) ? loop_continue  // prevent script from reading itself
  local(
    ...
    currentfile = file(#libs_path + loop_value + '.lasso'),
    currentfileroot = loop_value,
    typelist = array(),
    traitlist = array(),
    doc = file(#doc_src_path + loop_value + `.rst`)
//     handle => { #doc->close }
  )
  ...
  #doc->openTruncate&writeBytes(bytes(
    '='*#currentfileroot->size +
    '\n' + #currentfileroot +
    '\n' + '='*#currentfileroot->size
  ...
  where not #method->methodName->asString->contains('knop_user')
  ...
  order by #method->methodName->asString
  do {
    #doc->writeBytes(bytes(writeDocs(#method, 'method')))
  }
  ...
  do {
    #doc->writeBytes(bytes(writeDocs(#trait, 'trait')))
  }
  #doc->close
}
...
Knop Web Application Framework for Lasso 9
==========================================
